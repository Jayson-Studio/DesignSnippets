...
4. Open a blank TextEdit document, type `Update the border to #border`, and use Up/Down to navigate entries, Left/Right to switch tabs, then Return to choose `--border-default`.
5. Suggestions open above the `#` (below if there is insufficient space). Continue typing to filter names; Backspace broadens the results. If caret/glyph bounds are unavailable, placement is approximate: the last click in the active app, or the pointer position at opening if no click was recorded. Drag the top edge or bottom search row of the picker to choose a position; DesignSnippets remembers it for that app and display and uses it the next time cursor bounds are unavailable. Right-click the search row to reset. Saved positions stay within the display after rearrangement/resizing, but do not track a moving editor window or caret. Without a verified selection range, Return copies the token for manual pasting rather than replacing text.
...
The picker remembers whether you started or paused it and resumes on launch when Accessibility access is granted. It works across compatible apps by default; Preferences can optionally restrict it to selected apps. It refuses detected secure fields and secure event input. In supported editors, automatic insertion uses a recognized editable field and a verified trigger range. In Codex, Enter replaces an uninterrupted tracked #query containing ASCII token characters; navigation, shortcuts, outside clicks, or app switches cancel the session. Composed or non-ASCII input stays copy-only. DesignSnippets reads available Accessibility data without setting the destination app's accessibility flags. Codex uses a draggable picker exclusively. It restores the saved position for the display under the pointer, or opens above the last click in the app (or the pointer if no click was recorded), falling below when space is limited; drag its top edge or bottom search row to reposition and right-click to reset. It does not query Codex caret bounds or attempt text-marker positioning. Other apps retain standard cursor positioning. Electron apps and web editors vary in how they expose Accessibility, so **ChatGPT, Codex, Cursor, and VS Code are target integrations, not certified compatibility claims**. The picker never presses Return in the destination. Automatic insertion preserves the clipboard; copy mode writes the selected token to it.
